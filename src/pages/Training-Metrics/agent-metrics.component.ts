...
// ── Adjust this to your real service path ────────────────────────────────────
import {
  MarlsDashboardService,
  MarlsVM, AgentVM, AgentDecision, TrainingStep, DashboardApiResponse,
...
export class AgentMetricsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Prometheus queries actually executed (from collector.go)
  readonly promQueries = [
    {
      metric: 'CPU Usage',
      color: '#22d3ee',
  ...
  ngOnInit(): void {
    ...
    this.subs.add(this.svc.dashboard$.subscribe((vm: MarlsVM | null) => {
      ...
      if (vm) {
        this.connected = true;
        this.loading   = false;
        // Populate agent keys on first load
  ...
  private refreshAgent(vm: MarlsVM): void {
    ...
    this.decisionHistory = (agent.decisionHistory ?? [])
      .slice(0, 50)
      .map((d: AgentDecision, idx: number) => ({
        action:          d.action,
        replicas:        d.replicas,
        reward:          d.reward,
    ...
    // Build the 18-dim state vector display from live agent metrics
    this.stateVector = this.buildStateVector(agent);

    // Reward components from ppo_agent._raw_reward logic
    this.rewardComponents = this.buildRewardComponents(agent);
    this.totalReward = this.rewardComponents.reduce((s: number, r: RewardComponent) => s + r.value, 0);
  ...
  private buildStateVector(a: AgentVM): StateItem[] {
    const lat   = a.latencyMs / 1000;   // back to seconds
    const sla   = 0.5;
  ...
  private buildRewardComponents(a: AgentVM): RewardComponent[] {
  ...
  // ── Line chart (reused from dashboard) ────────────────────────────────────
